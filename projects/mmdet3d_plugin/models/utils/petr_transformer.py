...
class PETRTransformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed, reg_branch=None,
                query_content=None, query_key_padding_mask=None):
        ...
        if query_content is None:
            ...
            target = query_content.unsqueeze(1).repeat(1, bs, 1)
        elif query_content.dim() == 3:
            target = query_content.permute(1, 0, 2).contiguous()
        else:
            raise ValueError(f'Unsupported query_content shape: {query_content.shape}')
