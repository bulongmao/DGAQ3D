...
# Clean 3DPPE 1600x640 train-only keyframe + DenseGT depth-supervision variant.
#
# It inherits the clean keyframe baseline behavior, then replaces the sparse
# LiDAR-projected depth target with offline dense depth maps. This file is only
# for baseline-vs-DenseGT comparison; it is not the pure official baseline.
# Training is explicitly train-only to keep val-set ablation clean.
...
_dense_depth_root = 'data/metric3d_depth'
ida_aug_conf = {
    'resize': (-0.06, 0.11),
    'rot': (0.0, 0.0),
    'flip': True,
